...
const string strMessageMagic = "K2Coin Signed Message:\n";
...
{
    // K2Coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // K2Coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xd4, 0xcb, 0xa6, 0xe7 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("K2CoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("K2CoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static K2CoinMiner(CWallet *pwallet)
{
    printf("K2CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("K2Coin-miner");
    ...
    try { loop {
        ...
        printf("Running K2CoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&K2CoinMiner, pwallet));
}
