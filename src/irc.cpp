...
// Copyright (c) 2011-2012 K2Coin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #K2CoinTEST0\r");
            Send(hSocket, "WHO #K2CoinTEST0\r");
        } else {
            // randomly join #K2Coin00-#K2Coin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #K2Coin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #K2Coin%02d\r", channel_number).c_str());
        }
